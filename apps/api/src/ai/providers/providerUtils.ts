...
export function buildProviderClient(config: ProviderConfig, apiKey: string) {
  ...
  return {
    ...
    async scoreParaphrase(input: { referenceText: string; transcript: string; threshold: number }) {
      const content = await send(`Score this student paraphrase against the reference text. The reference text may be a simplified version of the original, so judge the student on whether they captured the key ideas and concepts — not on whether they used specific vocabulary from the reference. Accept any wording, synonyms, or phrasing that conveys the same meaning. Do not list specific words or phrases from the reference as "missed" if the student expressed the same idea in different words. If all core ideas are present, score at or above the passing threshold. Return only valid JSON matching this shape: {"score":number,"rubric":{"mainIdea":number,"keyDetails":number,"context":number,"whyItMatters":number,"accuracy":number},"understood":["string"],"missed":["string"],"feedback":"string"}.\n\nPassing threshold: ${input.threshold}\n\nReference text:\n${input.referenceText}\n\nStudent paraphrase:\n${input.transcript}`);
